...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-4xl font-bold mb-8 text-center">Order Management</h1>
    @if($orders->isEmpty())
                    ...
                    <div class="bg-white shadow-md rounded-lg p-6 transition-transform transform hover:scale-105">
                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
                            <div>
                                ...
                                <p class="text-gray-500">Ordered by {{ $order->user->name }}</p>
                            </div>
                        ...
                        <!-- Update Status Form -->
                        <div class="mt-6">
                            <form action="{{ route('admin.orders.updateStatus', $order->id) }}" method="POST" class="flex space-x-2 items-center">
                                @csrf
                                <select name="status" class="border rounded p-1" required>
                                ...
                                <button type="submit" class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600 transition duration-300">Update</button>
                            </form>
                        </div>
                    </div>
                @endforeach
